...
bool ImGui::SwitchButton(const char* str_id, bool* v)
{
    ...
    if (IsItemHovered()) {
        draw_list->AddRectFilled(ImVec2(p.x,p.y+10), ImVec2(p.x + width, p.y + height+10), GetColorU32(*v ? colors[ImGuiCol_ButtonActive] : ImVec4(0.78f, 0.78f, 0.78f, 1.0f)), height * 0.5f);
    } else {
        draw_list->AddRectFilled(ImVec2(p.x,p.y+10), ImVec2(p.x + width, p.y + height+10), GetColorU32(*v ? colors[ImGuiCol_Button] : ImVec4(0.85f, 0.85f, 0.85f, 0.85f)), height * 0.50f);
    }
    draw_list->AddCircleFilled(ImVec2(p.x + radius + (*v ? 1 : 0) * (width - radius * 4.5f) + (t * 50), p.y + radius+10), radius - 5.f, IM_COL32(255, 255, 255, 255));
    RenderText(ImVec2(p.x + width * 1.12f,  p.y + radius-5 ), str_id);
